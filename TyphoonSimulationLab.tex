...
\begin{minipage}[r]{4.25in}{\small
  The enormous devastation caused by super typhoon Haiyan (2013) motivates us to consider whether data from past typhoons hold a key to predicting the category and maximum wind speed from an early description of an emerging tropical storm.  Various machine learning (ML) classification algorithms to predict the typhoon grade are assessed using Confusion Matrices and regression algorithms used to predict maximum wind speed via residuals.}
\end{minipage}
...
\section{MATLAB Tree Model Output}

Using MATLAB's classification app (included in the Statistics and Machine Learning ) the confusion matrix (Figure \ref{fig1}) indicates that 47 Grade 5 typhoons are correctly predicted, with 13 false positive (8 Grad 4 and 5 Grade 3) and 16 False Negative (7 predicted as Grade 3 and 9 as Grade 4).  
...
\begin{enumerate}
\item Which MATLAB algorithm gives the best classification?  The lowest RMSE wind-speed prediction error?
\item Write a script which performs a typhoon grade classification prediction and outputs a confusion matrix. What is the basis for your classification algorithm?
\end{enumerate}
